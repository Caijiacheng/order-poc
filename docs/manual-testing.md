...
- 打开右下角 `AI 下单助手` 后，侧栏按“顶部信息区 -> 当前采购情况 -> 你可以这样操作 -> 补充偏好 -> 过程区 -> 结果区”组织，不出现 `AutofillProgressCard`、`AutofillResultCard` 等技术词。
- 顶部状态显示当前金额、已选 SKU、本周可参与活动；`查看模板` 与 `查看活动` 可点击并回到页面对应区域。
- `一键做单` 可在无额外文本输入时直接生成采购预览；确认前购物车数量和金额不变。
- `活动补齐`、`解释这单`、快捷偏好 chips、`发送`、`停止` 等操作可点击。
- 可通过 `上传图片` 或 `粘贴截图` 添加图片，图片缩略图会显示在输入区。
- 点击任一图片缩略图会打开预览弹窗；多图时支持上一张/下一张；支持关闭按钮、点击遮罩、按 `Esc` 关闭。
- 图片识别完成后会出现轻量识别摘要；`查看识别内容` 可打开识别明细，不与图片预览混用。
- 图片仅作为输入方式参与本次做单，最终仍然是“生成采购预览 -> 用户确认加入采购清单”。
...
- 右下角入口同样命名为 `AI 下单助手`，但侧栏只支持 `解释当前优化`、`继续安全补齐`、`去提交`。
- 结算页 Copilot 不出现图片上传入口、图片缩略图、图片预览弹窗或识别明细入口。
- `继续安全补齐` 仍保持预览优先，确认前不直接改写当前清单。
...
- AI 下单助手运行视图支持按页面、状态、类型、输入方式（文字 / 图片 / 混合）筛选。
...
- AI 下单助手链路仅保留 `/purchase` 与 `/order-submit` 两类主轨迹；可按输入方式筛选，并查看 OCR/图片摘要元数据。
