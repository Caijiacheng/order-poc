...
import type { CartSession } from "@/lib/memory/types";

type OrderSubmitCopilotPanelProps = {
  customerId: string;
  customerName?: string;
  cart: CartSession | null;
  optimization: CartOptimizationResponse | null;
  onApplySuccess?: (result: CopilotApplyDraftResponse) => Promise<void> | void;
};

const STEP_LABEL_MAP: Record<string, string> = {
  load_context: "读取结算上下文",
  parse_intent: "解析补齐意图",
  detect_campaign_state: "识别活动状态",
  build_legal_candidates: "生成安全候选",
  select_best_combo: "选择最佳补齐组合",
  apply_draft: "生成预览草稿",
...
function toBlockedReasonText(reason?: string) {
  if (!reason) {
    return "当前约束下未找到可执行的安全补齐组合。";
  }
  if (reason === "no_legal_combo") {
    return "未找到符合规则的补齐组合，请调整约束后重试。";
  }
...
export function OrderSubmitCopilotPanel(props: OrderSubmitCopilotPanelProps) {
  const [mounted, setMounted] = useState(false);
  const [open, setOpen] = useState(false);
  const [runningAction, setRunningAction] = useState<"topup" | null>(null);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      try {
        ...
        // Keep current state if polling fails transiently.
      }
    }, 1200);
  ...
  const runSafeTopup = async () => {
    if (!hasCustomerContext) {
      setPanelError("当前购物车未绑定经销商，暂无法执行补齐。");
      return;
    }
  ...
  const runExplain = () => {
    if (!hasCustomerContext) {
      setPanelError("当前购物车未绑定经销商，暂无法解释。");
      return;
    }
  ...
  const handleGoSubmit = () => {
    setOpen(false);
    const submitButton = document.getElementById("order-submit-primary-button");
  ...
  const recommendationBarCount = props.optimization?.recommendationBars.length ?? 0;
  const cartSummary = props.cart?.summary;
