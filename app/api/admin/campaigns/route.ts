...
import { createCampaign, listCampaigns } from "@/lib/admin/service";
...
export async function GET(request: Request) {
  try {
    ...
    const data = listCampaigns(query);
...
export async function POST(request: Request) {
  try {
    ...
    const validation = validateCampaignInput(payload, "create");
    if (!validation.valid) {
      return apiError(
        "VALIDATION_ERROR",
        "参数校验失败",
    ...
    const data = createCampaign(validation.value);
