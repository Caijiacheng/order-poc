...
export default function BasketPage() {
  ...
  const recomputeOptimization = async (options?: { silent?: boolean }) => {
    ...
    try {
      ...
      setOptimization(null);
      setErrorMessage(error instanceof Error ? error.message : "自动计算优化建议失败");
    } finally {
      if (latestOptimizationToken.current === token) {
        setOptimizing(false);
      }
  ...
  }, [loadingPage, dealerId, cartSignature]);
  ...
  const applyAllOptimizations = async () => {
    ...
    try {
      ...
      await reloadCart();
      setSuccessMessage("已批量应用本轮自动优化建议。");
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <section className="space-y-3">
        ...
        <p className="text-sm text-slate-600">系统会自动按金额、箱规、搭配三维度更新建议，你只需要决定是否应用。</p>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
        <Card className="border-slate-200 bg-white/92">
          <CardHeader className="space-y-2">
            ...
            <p className="text-sm text-slate-600">支持改量、删项；每次变化后系统自动重算优化建议。</p>
          </CardHeader>
          <CardContent>
            {loadingPage || !cart ? (
                      ...
                      <TableCell className="text-right">
                        <div className="inline-flex items-center gap-2">
                          <Input
                            ...
                            onChange={(event) => setQtyDraft((prev) => ({ ...prev, [row.sku_id]: event.target.value }))}
                          />
                          <Button size="sm" variant="outline" onClick={() => updateItemQty(row.sku_id)} disabled={busyActionKey === `patch:${row.sku_id}`}>
                            更新
                          </Button>
                        </div>
                      </TableCell>
                      <TableCell className="text-right">
                        <Button size="sm" variant="destructive" onClick={() => handleRemoveItem(row.sku_id)} disabled={busyActionKey === `remove:${row.sku_id}`}>
                          删除
                        </Button>
                      </TableCell>
                    </TableRow>
        ...
        <Card className="h-fit border-slate-200 bg-gradient-to-b from-slate-50 to-white lg:sticky lg:top-24" data-testid="basket-summary">
          <CardHeader>
          ...
          <CardContent className="space-y-3 text-sm">
            {!optimization?.thresholdSuggestion ? (
              <div className="rounded-xl border border-dashed border-slate-300 p-3 text-slate-500">
                {optimization ? "当前清单已满足门槛，无需补齐。" : "系统将自动计算门槛补齐建议。"}
              </div>
            ) : (
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="font-medium text-slate-900">{optimization.thresholdSuggestion.reason}</p>
                <p className="mt-1 text-xs text-slate-600">
                  {optimization.thresholdSuggestion.sku_id} · 建议 {optimization.thresholdSuggestion.suggested_qty} 箱
                </p>
                <Button
                  size="sm"
                  className="mt-3"
                  onClick={applyThresholdSuggestion}
                  disabled={busyActionKey === `apply:threshold:${optimization.thresholdSuggestion.sku_id}`}
                >
                  应用建议
                </Button>
              </div>
              ...
              optimization.boxAdjustments.map((item) => (
                <div key={item.sku_id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="mt-3"
                    onClick={() => applyBoxAdjustment(item.sku_id, item.to_qty, item.recommendation_item_id)}
              ...
              optimization.pairSuggestions.map((item) => (
                <div key={item.sku_id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="mt-3"
                    onClick={() => applyPairSuggestion(item.sku_id, item.suggested_qty, item.recommendation_item_id)}
